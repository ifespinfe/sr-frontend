...
const UsernameSchema = string().required("Username is required").max(24, "Username too long").matches(/^\S*$/, "Username should not have space");
const BioSchema = string().optional().max(350, "Limit bio to just 350 characters");
const passwordSchema = string().required("Enter your password").min(8, "Password should be at least 8 characters long").matches(/[0-9]/, "Password should contain at least one number").matches(/[a-z]/, "Password should contain at least one lowercase letter").matches(/[A-Z]/, "Password should contain at least one uppercase letter").matches(
  /[!@#$%^&*(),.?":{}|<>]/,
  "Password should contain at least a special character"
);
const AudienceSchema = object({
...
const HostSchema = object({
  name: string().optional().min(3, "Name should be at least 3 characters long"),
  email: string().required("Email is required").email("Enter a valid email"),
  stage_name: string().required("Stage name is required").min(2, "Stage name should be at least 2 characters long"),
...
export { AudienceSchema as A, BioSchema as B, ChangePasswordSchema as C, HostSchema as H, ResetPasswordSchema as R, UsernameSchema as U };
//# sourceMappingURL=user-schema-Imz6LROc.mjs.map
